...
Use exposed browsing/file-fetch tools or available shell HTTP access. Every upstream request must use the pinned commit, never `main`, a latest-branch redirect, or a search-engine summary. Resource-backed content, attachments, or a cache are acceptable only when provenance verifies this exact repository, commit, and file path; an arbitrary uploaded lookalike is not a pinned example. Read host-managed resources with their actual identifiers. Store files, if needed, in a writable project/session output area, never in the installed plugin. Native browsing does not imply outbound Python networking.
...
3. Fetch the [example index](https://raw.githubusercontent.com/alyshadotmd/visual-formats/498444b7fdc4da674f07d8b0432c32a24b5062c9/files/_index.md). Find media tagged `vf=<token>`, including filenames with multiple format tags. Resolve media at repository-root `files/`, URL-encoding the filename. Visually inspect actual relevant still examples through remote-image viewing, provenance-verified resource/attachment viewing, or downloaded files. A local download is not mandatory when native vision can see the source image; filenames and text descriptions never count as visual inspection. Follow guide-specific example instructions; at least one inspected still example per selected format is required (a dual-tag example can cover both).
